...
def crearUsuario(di):
    ...
    val = (
        ...
        di.get('nombre_completo'),  # Asumiendo que el diccionario contiene 'nombre_completo'
        di.get('email'),
        di.get('legajo'),
        di.get('contraseña'),
        di.get('tipo_usuario')
    ...
    # Llamada a la función para realizar el insert
    resul_insert = insertDB(configDB, sQuery, val) #FIJARME EN LA PAGINA DE MYSQL CONNECTOR PARA ENCONTRAR INSERTDB
...
#def validarYAccederUsuario(request):
    email = request.form.get('email')
    password = request.form.get('password')
    legajo = request.form.get('legajo')


    print(f"Verificando usuario: {email}, {password}, {legajo}")  # Agregar esto para depurar

    # Consulta para validar usuario y tipo_usuario
    sql = """
        SELECT * 
        FROM usuario 
        WHERE email = %s AND password = %s AND legajo = %s AND tipo_usuario = 'profesor';
    """
    parametros = (email, password, legajo)
    resultado = selectDB(configDB=configDB, sql=sql, val=parametros)

    print(f"Resultado de la consulta: {resultado}")  # Agregar esto para depurar

    # Verifica que el usuario existe y cumple con el tipo_usuario
    return len(resultado) > 0
#def obtenerUsuarioPorCredenciales(email, password, legajo):
    """
    Consulta la base de datos para verificar si las credenciales ingresadas son válidas.
    """
    consulta_sql = """
        SELECT * 
        FROM usuarios 
        WHERE email = %s AND password = %s AND legajo = %s;
    """
    valores = (email, password, legajo)
    
    # Realizar la consulta a la base de datos
    resultado = selectDB(configDB={"host": "localhost", "user": "root", "password": "", "database": "seva"}, 
                         sql=consulta_sql, 
                         val=valores, 
                         title=True)
    
    # Verificar si hay un resultado válido
    if len(resultado) > 1:  # La primera fila contiene los títulos
        return resultado[1]  # Devolver la primera fila de datos
    return None
