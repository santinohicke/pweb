...
def obtenerComisiones(id_profe):
    val= seleccionComisiones(id_profe)
    lst=[]
    for item in val:
...
def obtenerinscripcion(id_alumno):
    ...
    for item in inscripciones:
        comision = encontrarComisionAlumno(item[-1])
...
def datosInscripcion(diRequest):
    diRequest['materias'] = request.form.getlist('materias[]')
    diRequest['nombre'] = request.form.get('nombre')  
    diRequest['email'] = request.form.get('email')
